...
const ProgressLine = ({ isActive }) => {
  return (
    <div className="absolute top-[50%] left-0 h-1 w-[100%] border-3 translate-y-[50%]">
    </div>
  );
};
...
const ProgressBar = ({ noOfScreens, onPressButton }) => {
  return (
    <div className="flex justify-center items-center flex-row mt-10">
      ...
      <Circle num={2} isActive={noOfScreens >= 2 ? true : false} />
      <ProgressLine isActive={true} />
      <Circle num={3} isActive={noOfScreens >= 3 ? true : false} />
      <ProgressLine isActive={true} />
      <Circle num={4} isActive={noOfScreens >= 4 ? true : false} />
    </div>
  );
};
